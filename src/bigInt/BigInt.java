...
public class BigInt implements Number {
	...
	public BigInt add(BigInt b) {
		...
		LinkedList<String> t = this.number;
		LinkedList<String> o = b.number;
		Node<String> i = t.root;
		Node<String> j = o.root;
		String result = "";
		int num;
		int carry = 0;
	...
	public BigInt divide(BigInt b){
		if(b.toString().equalsIgnoreCase("0")){
			try{
				int i = 1/0;
				i = i+0;
				return null;
			...
			BigInt curInc = new BigInt("");
			BigInt curNum = new BigInt("0");
			...
			while(count >= 0){
				...
				//apply any remainder and then add the next most sig digit
				
				if(!remainder.negative && !remainder.toString().equalsIgnoreCase("0")){
					curNum = remainder;
				}else{
					remainder = new BigInt("0");
				}
				if(curNum.toString().equalsIgnoreCase("0")){
					curNum.number.delete();
				}
				curNum.number.addNewRoot(i.getInfo());
				...
				if(curInc.greaterThan(curNum) && multiplier != 0){
					multiplier--;
					curInc = b.multiply(new BigInt("" + multiplier));
				}
				
				answer.number.addNewRoot("" + multiplier);
				multiplier = 0;
				
				if(answer.toString().equalsIgnoreCase("0")){
				...
				if(curInc.toString().equalsIgnoreCase(curNum.toString())){
					curNum = new BigInt("0");
				}
			}
			cleanUp(answer);
	...
	public void cleanUp(BigInt b){
		LinkedList<String> t = b.number;
		Node<String> pointer = t.root;
		boolean again = true;
		while(again){
			again = false;
			pointer = t.root;
	...
	public String toString(){
		Node<String> pointer = number.root;
		String s = "";
		int count = 0;
		while(pointer != null ){
			if(count % 3 == 0
